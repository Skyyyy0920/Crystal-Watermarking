...
class BaseModule(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        # TODO: val_loss seems not working
...
class CDVAE(BaseModule):
    def __init__(self, *args, **kwargs) -> None:
        ...
        self.encoder = hydra.utils.instantiate(self.hparams.encoder, num_targets=self.hparams.latent_dim)
        ...
        if self.hparams.predict_property:
            self.fc_property = build_mlp(self.hparams.latent_dim, self.hparams.hidden_dim, self.hparams.fc_num_layers,
                                         1)
    ...
    def langevin_dynamics(self, z, ld_kwargs, gt_num_atoms=None, gt_atom_types=None):
        ...
        output_dict = {'num_atoms': num_atoms, 'lengths': lengths, 'angles': angles,
                       'frac_coords': cur_frac_coords, 'atom_types': cur_atom_types, 'is_traj': False}

        if ld_kwargs.save_traj:
            output_dict.update(dict(
                all_frac_coords=torch.stack(all_frac_coords, dim=0),
                all_atom_types=torch.stack(all_atom_types, dim=0),
                all_pred_cart_coord_diff=torch.stack(all_pred_cart_coord_diff, dim=0),
    ...
    def generate_rand_init(self, pred_composition_per_atom, pred_lengths, pred_angles, num_atoms, batch):
    ...
    def training_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        ...
        outputs = self.forward(batch, teacher_forcing, training=True)
    ...
    def validation_step(self, batch: Any, batch_idx: int) -> torch.Tensor:
        outputs = self.forward(batch, teacher_forcing=False, training=False)
    ...
    def compute_stats(self, batch, outputs, prefix):
        ...
        if prefix != 'train':
            ...
            loss = self.hparams.cost_coord * coord_loss + self.hparams.cost_type * type_loss
