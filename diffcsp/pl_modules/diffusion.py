...
class CSPDiffusion(BaseModule):
    ...
    def forward(self, batch):
        ...
        times_long = times.long()
        alphas_cumprod = self.beta_scheduler.alphas_cumprod[times_long]
        beta = self.beta_scheduler.betas[times_long]

        c0 = torch.sqrt(alphas_cumprod)
        c1 = torch.sqrt(1. - alphas_cumprod)

        sigmas = self.sigma_scheduler.sigmas[times_long]
        sigmas_norm = self.sigma_scheduler.sigmas_norm[times_long]
    ...
    @torch.no_grad()
    def sample(self, batch, l_T, x_T, step_lr=1e-5):
        batch_size = batch.num_graphs
        ...
        for t in tqdm(range(time_start, 0, -1)):
            ...
            x_t, l_t = traj[t]['frac_coords'], traj[t]['lattices']
